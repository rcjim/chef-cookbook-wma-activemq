...
include_recipe 'java::default' if node['wma-activemq']['install_java']

tmp = Chef::Config[:file_cache_path]
version = node['wma-activemq']['version']
mirror = node['wma-activemq']['mirror']
activemq_home = "#{node['wma-activemq']['home']}/apache-activemq-#{version}"

directory node['wma-activemq']['home'] do
  recursive true
end
...
template "#{activemq_home}/conf/activemq.xml" do
  ...
  owner 'root'
  group 'root'
  notifies :restart, 'service[activemq]' if node['wma-activemq']['enabled']
  only_if  { node['wma-activemq']['use_default_config'] }
end

service 'activemq' do
  supports restart: true, status: true
  action [:enable, :start]
  only_if { node['wma-activemq']['enabled'] }
end
